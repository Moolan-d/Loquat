...
<p align="center">
  <img src="screenshot1.webp" alt="Loquat 翻译弹窗" width="600" />
</p>
...
- **即时弹窗** — 一个全局快捷键即可从菜单栏唤出热弹窗，结果即刻呈现，无需切换应用。
- **双引擎并行** — Google 翻译 + 任意 OpenAI 兼容大模型（OpenAI、DeepSeek、OpenRouter，或自建端点）。两个引擎独立翻译、独立成败、独立重试，一个出错不会遮住另一个的结果。
- **智能方向检测** — 含汉字的输入解析为「中文 → 英文」，其余解析为「英文 → 中文」。可一键覆盖或对调方向。

### 输入

- **手动输入** — 随时输入或粘贴任意文本。
- **剪贴板随快捷键** — 通过全局快捷键唤出弹窗时，可自动读取剪贴板：不超过 500 字符的文本立即翻译，更长的文本只填入输入框、等你按回车确认，绝不意外消耗 API 额度。
- **菜单栏点击永不读剪贴板** — 普通复制就是普通复制。

### 设置

- 自定义全局快捷键（录制任意组合键）。
- Provider 按 **Google** 与 **LLM** 分组，各自带翻译窗口的显示开关。
- 每个 provider 的连通性测试。
- LLM 提示词预设（通用 / 技术研发）。
- 登录时启动。
...
- API 密钥只存于 macOS **钥匙串**——绝不写入偏好设置、日志或请求 URL。
- 无统计、无遥测、无第三方运行时依赖。
- 远程 LLM 端点必须使用 HTTPS（本地模型允许 localhost）。
...
- **剪贴板读取只在你要的时候发生** — 把剪贴板翻译绑定到快捷键，让无关的复制不会触发付费请求。
- **小巧原生** — 纯 Swift，无 Electron、无 WebView。
- **独立失败** — 一个引擎出错不会使另一个引擎的成功结果失效。

## 性能

- **下载大小** — `Loquat-macOS.zip` 约 1.4 MB。
- **安装后大小** — 解压约 3 MB。
- **空闲内存** — 每进程物理内存占用 ≤ 50 MB，空闲 CPU 接近 0%（由发布门槛校验）。
